...
| `SWR-GUI-014` | Manual GUI review + supporting config tests | `ConfigTest.ReadabilityMode*` covers `monitor.cfg` defaults/preservation; rendered readability mode remains a visual verification item |
...
| GUI-MAN-07 | `SWR-GUI-014` | Enable readability mode, verify all six tiles, restart, then disable it again | Larger tile labels/values remain fully visible with unchanged values, units, badges, sparklines, and alert colours; the setting restores after restart and standard mode returns cleanly when disabled | |
...
| `tests/unit/test_config.cpp` (`ConfigTest`) | 13 | `SWR-GUI-014` persistence support plus supporting `monitor.cfg` checks |
| `tests/unit/test_localization.cpp` (`LocalizationTest`) | 9 | `SWR-GUI-012` |
| **Total** | **297** | |
...
| Criterion | Threshold |
|---|---|
| Unit test pass rate | 100% (`297 / 297`) |
| Integration test pass rate | 100% (`14 / 14`) |
| Automated GTest total | 100% (`311 / 311`) |
...
| F | 2026-05-06 | Codex implementer | Added readability-mode evidence expectations, GUI-MAN-07, and updated automated totals to 311 tests |
